...
from .helixer_model import HelixerModel, HelixerSequence
...
class HybridModel(HelixerModel):
    ...
    def model(self):
        ...
        if self.input_coverage:
            ...
            raw_input = Input(
                shape=(None, values_per_bp),
                dtype=self.float_precision,
                name="raw_input",
            )
            main_input, coverage_input = tf.split(
                raw_input, [4, 2 * self.coverage_count], axis=-1
            )
    ...
    def model_hat(self, penultimate_layers):
        ...
        if self.input_coverage:
            coverage_input = Reshape((-1, self.pool_size * self.coverage_count * 2))(
                coverage_input
            )
