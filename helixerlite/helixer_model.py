...
from .overlap import OverlapSeqHelper
...
class ConfusionMatrixTrain(Callback):
    def __init__(
        ...
        calc_H=False,
        report_to_nni=False,
        check_every_nth_batch=1_000_000,
        ...
        self.check_every_nth_batch = (
            check_every_nth_batch  # high default for ~ 1 / epoch
        )
        self.epoch = 0
    ...
    def check_in(self, batch=None):
        _, _, val_genic_f1 = HelixerModel.run_metrics(
            self.val_generator, self.model, calc_H=self.calc_H
        )
        ...
        if val_genic_f1 > self.best_val_genic_f1:
            ...
            print(
                "saved new best model with genic f1 of {} at {}".format(
                    self.best_val_genic_f1, self.save_model_path
                )
            )
            self.checks_without_improvement = 0
        else:
    ...
    def on_train_end(self, logs=None):
        if os.path.isdir(self.large_eval_folder):
            ...
            _, _, val_genic_f1 = HelixerModel.run_metrics(
                self.val_generator, best_model, print_to_stdout=True, calc_H=self.calc_H
            )
            assert val_genic_f1 == self.best_val_genic_f1

            training_species = self.train_generator.h5_file.attrs["genomes"]
            median_f1 = HelixerModel.run_large_eval(
                self.large_eval_folder, best_model, self.val_generator, training_species
            )
...
class HelixerSequence(Sequence):
    def __init__(self, model, h5_files, mode, batch_size, shuffle):
        ...
        if self.mode == "test":
            assert (
                len(self.h5_files) == 1
            ), "predictions and eval should be applied to individual files only"
        ...
        for cs in chunk_sizes[1:]:
            assert (
                cs == chunk_sizes[0]
            ), f"Not all subsequence lengths match in h5_files: {chunk_sizes}"
        ...
        if self.overlap:
            assert (
                self.mode == "test"
            ), "overlapping currently only works for test (predictions & eval)"
            # can take [0] below bc we've asserted that test means len(self.h5_files) == 1 above
            contiguous_ranges = get_contiguous_ranges(self.h5_files[0])
            self.ol_helper = OverlapSeqHelper(
        ...
        if self.input_coverage:
            self.data_list_names += [
                "evaluation/rnaseq_coverage",
                "evaluation/rnaseq_spliced_coverage",
            ]

        self.data_lists = [[] for _ in range(len(self.data_list_names))]
        self.data_dtypes = [
            self.h5_files[0][name].dtype for name in self.data_list_names
        ]

        self.compressor = numcodecs.blosc.Blosc(
            cname="blosclz", clevel=4, shuffle=2
        )  # use BITSHUFFLE
        ...
        if self.mode == "test":
            if self.class_weights is not None:
                print(
                    f'ignoring the class_weights of {self.class_weights} in mode "test"'
                )
                self.class_weights = None
            if self.transition_weights is not None:
                print(
                    f'ignoring the transition_weights of {self.transition_weights} in mode "test"'
                )
    ...
    def _load_one_h5(self, h5_file):
        ...
        if self.mode == "train" or self.mode == "val":
            mask = np.logical_and(
                h5_file["data/is_annotated"], h5_file["data/err_samples"]
            )
        ...
        for name, data_list in zip(self.data_list_names, self.data_lists):
            ...
            for offset in range(0, n_seqs, max_at_once):
                ...
                if name == "data/predictions":
                    data_slice = h5_file[name][0, offset : offset + max_at_once][
                        step_mask
                    ]  # only use one prediction for now
                else:
            ...
            if name not in self.data_list_names:
                ...
                if name == "data/X" and self.input_coverage:
                    decode_coverage = self.get_batch_of_one_dataset(
                        "evaluation/rnaseq_coverage", batch_idx
                    )
                    decode_coverage = [
                        self._cov_norm(x.reshape(-1, self.coverage_count)).astype(
                            np.float16
                        )
                        for x in decode_coverage
                    ]
                    decode_spliced = self.get_batch_of_one_dataset(
                        "evaluation/rnaseq_spliced_coverage", batch_idx
                    )
                    decode_spliced = [
                        self._cov_norm(x.reshape(-1, self.coverage_count)).astype(
                            np.float16
                        )
                        for x in decode_spliced
                    ]
                    decoded_list = [
                        np.concatenate((x, y, z), axis=1)
                        for x, y, z in zip(
                            decoded_list, decode_coverage, decode_spliced
                        )
                    ]
    ...
    def _decode_one(self, name, h5_indices):
        ...
        decoded_list = [
            np.frombuffer(self.compressor.decode(data_list[idx]), dtype=dtype)
    ...
    def _cov_norm(self, x):
        ...
        if method is None:
            ...
            raise ValueError(
                f"unrecognized method: {method} for normalizing coverage data"
            )
    ...
    def compress_tw(self, transitions):
        return self._squish_tw_to_sw(
            transitions, self.transition_weights, self.stretch_transition_weights
        )
    ...
    def _apply_stretch(reshaped_sw_t, stretch):
        ...
        for z in range(stretch, 0, -1):
            dilated_rf[i, np.maximum(np.subtract(j, z), 0)] = np.maximum(
                reshaped_sw_t[i, j] / dividers[z - 1], 1
            )
            dilated_rf[i, np.minimum(np.add(j, z), len(dilated_rf[0]) - 1)] = (
                np.maximum(reshaped_sw_t[i, j] / dividers[z - 1], 1)
            )
        ...
        if pool_size > 1:
            ...
            if self.mode == "train":
                if self.class_weights is not None:
                    ...
                    cls_arrays = [
                        np.any((y[:, :, :, col] == 1), axis=2) for col in range(4)
                    ]
                    cls_arrays = np.stack(cls_arrays, axis=2).astype(np.int8)
                    # add class weights to applicable timesteps
                    cw_arrays = np.multiply(
                        cls_arrays, np.tile(self.class_weights, y.shape[:2] + (1,))
                    )
                ...
                if self.coverage_weights:
                    coverage_scores = coverage_scores.reshape(
                        (coverage_scores.shape[0], -1, pool_size)
                    )
                    # maybe offset coverage scores [0,1] by small number (bc RNAseq has issues too), default 0.0
                    if self.coverage_offset > 0.0:
...
class HelixerModel(ABC):
    def __init__(self, cli_args=None):
        self.cli_args = (
            cli_args  # if cli_args is None, the parameters from sys.argv will be used
        )

        self.parser = argparse.ArgumentParser()
        self.parser.add_argument("-d", "--data-dir", type=str, default=None)
        self.parser.add_argument(
            "-s", "--save-model-path", type=str, default="./best_model.h5"
        )
        self.parser.add_argument(
            "--large-eval-folder", type=str, default="", help=argparse.SUPPRESS
        )
        # training params
        ...
        self.parser.add_argument(
            "--stretch-transition-weights", type=int, default=0, help=argparse.SUPPRESS
        )  # bc no clear effect on result quality
        self.parser.add_argument(
            "--coverage-weights", action="store_true", help=argparse.SUPPRESS
        )
        self.parser.add_argument(
            "--coverage-offset", type=float, default=0.0, help=argparse.SUPPRESS
        )
        self.parser.add_argument(
            "--calculate-uncertainty", action="store_true", help=argparse.SUPPRESS
        )
        self.parser.add_argument(
            "--no-utrs", action="store_true", help=argparse.SUPPRESS
        )  # is this still needed?
        self.parser.add_argument("--predict-phase", action="store_true")
        self.parser.add_argument(
            "--load-predictions", action="store_true", help=argparse.SUPPRESS
        )  # bc no models that can use this are available
        ...
        self.parser.add_argument(
            "-p", "--prediction-output-path", type=str, default="predictions.h5"
        )
        self.parser.add_argument(
            "--compression",
            default="gzip",
            help="compression used for datasets in predictions "
        ...
        self.parser.add_argument("--cpus", type=int, default=1)
        self.parser.add_argument(
            "--gpu-id",
            type=int,
            default=-1,
        ...
        tuner = self.parser.add_argument_group(
            "fine tuning",
    ...
    def parse_args(self):
        ...
        self.only_predictions = (
            self.testing and not self.eval
        )  # do only load X in this case
        ...
        assert not (
            self.resume_training and (not self.load_model_path or not self.data_dir)
        )

        self.class_weights = eval(self.class_weights)
        if not isinstance(self.class_weights, (list, np.ndarray, type(None))):
            raise ValueError(
                f"--class-weights evaluated to {self.class_weights} of type {type(self.class_weights)}; "
        ...
        if not isinstance(self.transition_weights, (list, np.ndarray, type(None))):
            raise ValueError(
                f"--transition-weights evaluated to {self.transition_weights} "
                f"of type {type(self.transition_weights)}; "
                f"this commonly means you need to remove nested quotes if not starting with nni"
            )
        if type(self.transition_weights) is list:
            self.transition_weights = np.array(
                self.transition_weights, dtype=np.float32
            )

        if self.verbose:
            print("HelixerModel config:")
            pprint(args)
    ...
    def generate_callbacks(self, train_generator):
        callbacks = [
            ConfusionMatrixTrain(
                ...
                train_generator,
                self.gen_validation_data(),
                self.large_eval_folder,
                self.patience,
                calc_H=self.calculate_uncertainty,
    ...
    def run_metrics(generator, model, print_to_stdout=True, calc_H=False):
        ...
        metrics_calculator = Metrics(
            generator, print_to_stdout=print_to_stdout, skip_uncertainty=not calc_H
        )
    ...
    @staticmethod
    def run_large_eval(
        folder, model, generator, training_species, print_to_stdout=False, calc_H=False
    ):
        ...
        for i, eval_file_name in enumerate(eval_file_names):
            ...
            perf_one_species = HelixerModel.run_metrics(
                gen, model, print_to_stdout=print_to_stdout, calc_H=calc_H
            )
        ...
        print_table(
            results_by_name, "Generalization Validation by Name", training_species
        )
        print_table(
            results_by_f1, "Generalization Validation by Genic F1", training_species
        )

        # print one number summaries
        f1_scores = np.array([r[1][2] for r in results])
        in_train = np.array(
            [r[0].lower() in training_species for r in results], dtype=np.bool
        )
        table = [["Metric", "All", "Training", "Evaluation"]]
        for name, func in zip(
            ["Median F1", "Average F1", "Stddev F1"], [np.median, np.mean, np.std]
        ):
            table.append(
                [
                    name,
    ...
    def open_data_files(self):
        def get_n_correct_seqs(h5_files):
            ...
            for h5_file in h5_files:
                if "err_samples" in h5_file["/data"].keys():
                    ...
                    n_correct = np.count_nonzero(err_samples is False)
                    if n_correct == 0:
        ...
        def get_n_intergenic_seqs(h5_files):
            ...
            for h5_file in h5_files:
                if "fully_intergenic_samples" in h5_file["/data"].keys():
                    ...
                    n_fully_ig = np.count_nonzero(ic_samples is True)
                    if n_fully_ig == 0:
        ...
        if not self.testing:
            self.h5_trains = [
                h5py.File(f, "r")
                for f in glob.glob(os.path.join(self.data_dir, "training_data*h5"))
            ]
            self.h5_vals = [
                h5py.File(f, "r")
                for f in glob.glob(os.path.join(self.data_dir, "validation_data*h5"))
            ]
            try:
                self.shape_train = self.sum_shapes(
                    [h5["/data/X"] for h5 in self.h5_trains]
                )
            except IndexError as e:
                print(
                    "debugging info: self.h5_trains = {}, self.data_dir = {}".format(
                        self.h5_trains, self.data_dir
                    ),
                    file=sys.stderr,
                )
                raise e
            try:
                ...
                print(
                    "debugging info: self.h5_vals = {}, self.data_dir = {}".format(
                        self.h5_vals, self.data_dir
                    ),
                    file=sys.stderr,
                )
                raise e
            ...
            self.h5_tests = [
                h5py.File(self.test_data, "r")
            ]  # list for consistency with train/val
        ...
        if self.verbose:
            ...
            if not self.testing:
                ...
                print(
                    "Fully correct train/val seqs: {:.2f}% / {:.2f}%\n".format(
    ...
    def _make_predictions(self, model):
        ...
        for batch_index in range(len(test_sequence)):
            ...
            try:
                ...
                print(
                    "Errors at prediction often result from exhausting the GPU RAM."
            ...
            for dset_name, pred_dset in zip(output_names, predictions):
                ...
                if pred_dset.shape[2] != label_dim:
                    ...
                    if n_removed > 0:
                        zero_padding = np.zeros(
                            (pred_dset.shape[0], n_removed, pred_dset.shape[2]),
                ...
                if self.overlap:
                    pred_dset = test_sequence.ol_helper.overlap_predictions(
                        batch_index, pred_dset
                    )
    ...
    def _print_model_info(self, model):
        ...
        try:
            ...
            print(
                "An error occurred while running a subprocess, unable to record loaded_model_hash"
            )
        ...
        if not self.testing:
            if self.resume_training:
                if not self.fine_tune:
                    ...
                    if not self.input_coverage:
                        inp = oldmodel.input
                        output = self.model_hat(
                            (oldmodel.layers[dense_at - 1].output, None)
                        )
            ...
            if self.optimizer.lower() == "adam":
                self.optimizer = optimizers.Adam(
                    learning_rate=self.learning_rate, clipnorm=self.clip_norm
                )
            ...
            assert self.test_data.endswith(
                ".h5"
            ), "Need a h5 test data file when loading a model"
            ...
            if self.eval:
                ...
                _, _, _ = HelixerModel.run_metrics(
                    test_generator, model, calc_H=self.calculate_uncertainty
                )
                if self.large_eval_folder:
                    assert (
                        self.data_dir != ""
                    ), "need training data of the model for training genome names"
                    training_species = h5py.File(
                        os.path.join(self.data_dir, "training_data.h5"), "r"
                    ).attrs["genomes"]
                    _ = HelixerModel.run_large_eval(
                        self.large_eval_folder,
                        model,
                        test_generator,
                        training_species,
                ...
                if os.path.isfile(self.prediction_output_path):
                    print(
                        f"WARNING: {self.prediction_output_path} already exists and will be overwritten."
                    )
    ...
    def insert_coverage_before_hat(self, oldmodel, dense_at):
        ...
        raw_input = Input(
            shape=(None, values_per_bp), dtype=self.float_precision, name="main_input"
        )
